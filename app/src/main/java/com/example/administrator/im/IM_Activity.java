...
public class IM_Activity extends AppCompatActivity
{
    ...
    public void doLogin()
    {
        LoginInfo info=new LoginInfo("nanashi","19920124");
        //登录信息不变
        RequestCallback<LoginInfo> callback=
                new RequestCallback<LoginInfo>() {
                    @Override
                    public void onSuccess(LoginInfo param) {
                        Toast.makeText(IM_Activity.this,"登录成功",Toast.LENGTH_LONG).show();
                    }

                    @Override
                    public void onFailed(int code) {
                        Toast.makeText(IM_Activity.this,"登录失败",Toast.LENGTH_LONG).show();

                    }

                    @Override
                    public void onException(Throwable exception) {
                        Toast.makeText(IM_Activity.this,exception.toString(),Toast.LENGTH_LONG).show();
                    }
        };
